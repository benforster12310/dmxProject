...
- **timings** - this array contains a nested object for each timing with the key as the scene number and the property as the time to fire the scene in ms, starting at 0 for scene 0 - scene 0 will be created as a blank scene so that you may start from scene 1 with programming
